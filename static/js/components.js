var RoomForm = React.createClass({
  ...
  render: function(){
    return React.DOM.div({className: 'col-sm-8 col-sm-offset-2'},
                         ...
                         React.DOM.input({type: 'text', placeholder: 'Room Name',
                                         className: 'form-control', ref: 'roomName'}, null),
                         React.DOM.button({className: 'btn ntn-primary btn-block top-margin', 
                                          onClick: this.joinRoomHandler}, 'Join Room'),
                         React.DOM.ul(null,
                                      this.props.rooms.map(function(r){
                                        return React.DOM.li({className: 'list-unstyled'},
                                                            React.dom.a({href: '#room/' + r.get('name')},
                                                                        r.get('name'))
                                                            );
                                      })
                                      )); }
...
var UserView = React.createClass({
  render: function(){
    ...
    return React.DOM.div(null, React.DOM.img({src: this.props.user.image(this.props.size),
      className: 'img-circle', title: this.props.user.get('user')}),
      name
      )}
});
    ...
    return React.DOM.ul({className: 'list-unstyled'},
                        this.props.collection.map(function(user){
                          if(me.id !== user.get('id'))
                            return React.DOM.li(null, UserView({user: user, size: 50, 
                              useName: true}))
                        }))
  }
});
...
var ChatMessage = React.createClass({
  render:function(){
    ...
    return React.DOM.li(null,
                        React.DOM.div({className: 'bg-primary chat-message ' + pull},
                                      this.props.chat.get('message')),
                        React.DOM.div({className: 'clearfix'}, null),
                        React.DOM.div({className: pull},
                                      UserView({user: this.props.chat.get('user'),
                                               size: 20, useName: true}),
                                      React.DOM.small(null, timeAgo)),
                        React.DOM.div({className: 'clearfix'}, null)
                        )
  }
});
...
var ChatForm = React.createClass({
  ...
  render: function(){
   ...
   ref: "message"}, null)),
                                        Reasct.DOM.div({className: "col-sm-2"},
                                                       React.DOM.button({className: "btn btn-promary"}, 'Send'))
                                        ))}
});
